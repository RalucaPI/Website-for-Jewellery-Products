...
export function Bracelet() {
    ...
    const {
        productBratari, currentImages, setCurrentImages, setCautare, cautare,
        maxPretBratari, minPretBratari, rangeBratari, setRangeBratari, filteredProductBratari,
        reset, maxGramajBratari, minGramajBratari, favorite, handleAddToFavorites, handleAddToCart,
        rangeGramajBratari, setRangeGramajBratari, handleMaterialChange,
        handleColorChange,handleSizeChange, handleMouseLeave, handleMouseEnter,
    } = context;

    const materialValues = [...new Set(productBratari.map(product => product.material).filter(material => material))].sort();
    const colorValues = [...new Set(productBratari.map(product => product.culoare).filter(culoare => culoare))].sort();
    ...
    }, [filteredProductBratari]);

  


  const sizeValues = [...new Set(productBratari.flatMap(product => 
    ...
    return (
        <>
            <div>
                <h1 className='text-4xl text-center mb-14 mt-8 font-bold'>Bratari</h1>
            </div>
            ...
            <div className="flex">
                <div className="w-1/5 p-4">
                    <div className="filter_container">
                        ...
                        <Slider
                            ...
                            max={maxPretBratari}
                            value={rangeBratari}
                            onChange={setRangeBratari}
                            trackStyle={[{ backgroundColor: 'black' }]}
                            handleStyle={[{ borderColor: 'black' }, { borderColor: 'black' }]}
                            railStyle={{ backgroundColor: 'gray' }}
                        />
                        <div className="flex justify-between my-2">
                            <span>{rangeBratari[0]} Lei</span>
                            <span>{rangeBratari[1]} Lei</span>
                        </div>
                        Gramaj
                        <Slider
                            range
                            min={minGramajBratari}
                            max={maxGramajBratari}
                            value={rangeGramajBratari}
                            onChange={setRangeGramajBratari}
                            trackStyle={[{ backgroundColor: 'black' }]}
                            handleStyle={[{ borderColor: 'black' }, { borderColor: 'black' }]}
                            railStyle={{ backgroundColor: 'gray' }}
                        />
                        <div className="flex justify-between mt-2">
                            <span>{rangeGramajBratari[0]} g</span>
                            <span>{rangeGramajBratari[1]} g</span>
                        </div>
                ...
                <div className="w-4/5 p-4">
                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
                        {filteredProductBratari.map((product, index) => {
