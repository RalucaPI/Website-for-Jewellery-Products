...
export function Rings() {
    ...
    const {
        productInele, currentImages, setCurrentImages, setCautare, cautare,
        maxPretInele, minPretInele, rangeInele, setRangeInele, filteredProductInele,
        reset, maxGramajInele, minGramajInele, favorite, handleAddToFavorites, handleAddToCart,
        rangeGramajInele, setRangeGramajInele, handleMaterialChange,
        handleColorChange,handleSizeChange, handleMouseLeave, handleMouseEnter,
    } = context;

    const materialValues = [...new Set(productInele.map(product => product.material).filter(material => material))].sort();
    const colorValues = [...new Set(productInele.map(product => product.culoare).filter(culoare => culoare))].sort();
    ...
    }, [filteredProductInele]);

  


  const sizeValues = [...new Set(productInele.flatMap(product => 
    ...
    return (
        <>
            <div>
                <h1 className='text-4xl text-center mb-14 mt-8 font-bold'>Inele</h1>
            </div>
            ...
            <div className="flex">
                <div className="w-1/5 p-4">
                    <div className="filter_container">
                        ...
                        <Slider
                            ...
                            max={maxPretInele}
                            value={rangeInele}
                            onChange={setRangeInele}
                            trackStyle={[{ backgroundColor: 'black' }]}
                            handleStyle={[{ borderColor: 'black' }, { borderColor: 'black' }]}
                            railStyle={{ backgroundColor: 'gray' }}
                        />
                        <div className="flex justify-between my-2">
                            <span>{rangeInele[0]} Lei</span>
                            <span>{rangeInele[1]} Lei</span>
                        </div>
                        Gramaj
                        <Slider
                            range
                            min={minGramajInele}
                            max={maxGramajInele}
                            value={rangeGramajInele}
                            onChange={setRangeGramajInele}
                            trackStyle={[{ backgroundColor: 'black' }]}
                            handleStyle={[{ borderColor: 'black' }, { borderColor: 'black' }]}
                            railStyle={{ backgroundColor: 'gray' }}
                        />
                        <div className="flex justify-between mt-2">
                            <span>{rangeGramajInele[0]} g</span>
                            <span>{rangeGramajInele[1]} g</span>
                        </div>
                ...
                <div className="w-4/5 p-4">
                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
                        {filteredProductInele.map((product, index) => {
                            ...
                            return (
                                <div
                                    ...
                                    className="border-2 hover:shadow-2xl hover:scale-105 transition-shadow duration-300 ease-in-out
                                     border-[#cdc8ff] border-opacity-60 rounded-2xl overflow-hidden flex flex-col justify-between min-h-[370px] max-h-[420px]"
                                ...
                                >
                                    ...
                                    >
                                        <div className="h-[220px] flex justify-center items-center  mb-4">
                                            <Link to={`/produs/${id}`}>
                                                <img
