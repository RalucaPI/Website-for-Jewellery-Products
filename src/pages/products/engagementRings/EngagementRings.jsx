...
export function EngagementRings() {
    const [hoveredIndex, setHoveredIndex] = useState(null);
    const cardRefs = useRef([]);
    const context = useContext(Context);

    const {
        productVerighete, currentImages, setCurrentImages, setCautare, cautare,
        maxPretVerighete, minPretVerighete, rangeVerighete, setRangeVerighete, filteredProductVerighete,
        reset, maxGramajVerighete, minGramajVerighete, favorite, handleAddToFavorites, handleAddToCart,
        rangeGramajVerighete, setRangeGramajVerighete, handleMaterialChange,
        handleColorChange,handleSizeChange, handleMouseLeave, handleMouseEnter,
    } = context;

    const materialValues = [...new Set(productVerighete.map(product => product.material).filter(material => material))].sort();
    const colorValues = [...new Set(productVerighete.map(product => product.culoare).filter(culoare => culoare))].sort();
    ...
    }, [filteredProductVerighete]);

  


  const sizeValues = [...new Set(productVerighete.flatMap(product => 
    ...
    return (
        <>
            <div>
                <h1 className='text-4xl text-center mb-14 mt-8 font-bold'>Verighete</h1>
            </div>
            ...
            <div className="flex">
                <div className="w-1/5 p-4">
                    <div className="filter_container">
                        ...
                        <Slider
                            range
                            min={minPretVerighete}
                            max={maxPretVerighete}
                            value={rangeVerighete}
                            onChange={setRangeVerighete}
                            trackStyle={[{ backgroundColor: 'black' }]}
                            handleStyle={[{ borderColor: 'black' }, { borderColor: 'black' }]}
                            railStyle={{ backgroundColor: 'gray' }}
                        />
                        <div className="flex justify-between my-2">
                            <span>{rangeVerighete[0]} Lei</span>
                            <span>{rangeVerighete[1]} Lei</span>
                        </div>
                        Gramaj
                        <Slider
                            range
                            min={minGramajVerighete}
                            max={maxGramajVerighete}
                            value={rangeGramajVerighete}
                            onChange={setRangeGramajVerighete}
                            trackStyle={[{ backgroundColor: 'black' }]}
                            handleStyle={[{ borderColor: 'black' }, { borderColor: 'black' }]}
                            railStyle={{ backgroundColor: 'gray' }}
                        />
                        <div className="flex justify-between mt-2">
                            <span>{rangeGramajVerighete[0]} g</span>
                            <span>{rangeGramajVerighete[1]} g</span>
                        </div>
                ...
                <div className="w-4/5 p-4">
                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
                        {filteredProductVerighete.map((product, index) => {
